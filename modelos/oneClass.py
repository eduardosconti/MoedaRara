...
# Testa em uma única imagem
test_image = load_single_image(test_image_path)
if test_image is not None:
    test_image = scaler.transform(test_image)  # Aqui é onde você aplica a transformação
    prediction = oc_svm.predict(test_image)[0]
    label = "Normal" if prediction == 1 else "Anômala"
    
    test_image_reshaped = test_image.reshape(256, 256)
    
    # Exibe a imagem de teste original
    plt.figure(figsize=(10, 4))
    plt.subplot(1, 3, 1)
    plt.imshow(test_image_reshaped, cmap='gray')
    plt.title(f"Imagem de Teste - {label}")
